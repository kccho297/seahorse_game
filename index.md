...
            <div id="score-single" class="hidden text-5xl text-white font-bold drop-shadow-\\\[0\\\_4px\\\_4px\\\_rgba(0,0,0,0.8)] font-game">0m</div>
...
                <div class="bg-white/10 p-6 rounded-full shadow-\\\[0\\\_0\\\_50px\\\_rgba(56,189,248,0.3)]">
...
            PLAYER\_MAX\_SPEED: 5,

            PLAYER\_FRICTION: 0.92,

            PLAYER\_RADIUS: 20,

            ENERGY\_DECAY\_RATE: 0.03,

            ENERGY\_MOVE\_COST: 0.01,

            ENERGY\_CAMO\_DRAIN: 0.3,

            BASE\_SCROLL\_SPEED: 3.0,

            DIFFICULTY\_RAMP: 0.0005,

            MAX\_SPEED\_MULTIPLIER: 4.0,

            SPAWN\_RATES: { PLANKTON: 0.1, PLASTIC: 0.02, PREDATOR: 0.015, SEAGRASS: 0.03 },

            TRAWL\_INTERVAL: 1800,

            TRAWL\_WARNING\_DURATION: 180,

            TRAWL\_SPEED\_BASE: 5,

            TRAWL\_HEIGHT\_RATIO: 0.45,
...
                case 'plankton': w=10; h=10; vx = -CONFIG.BASE\_SCROLL\_SPEED\*speedMult; vy=(Math.random()-0.5); break;

                case 'plastic': w=25; h=25; vx = -CONFIG.BASE\_SCROLL\_SPEED\*speedMult\*1.2; vy=(Math.random()-0.5)\*0.5; break;

                case 'predator\_tuna': w=60; h=40; vx = -CONFIG.BASE\_SCROLL\_SPEED\*speedMult\*2.8; break;

                case 'predator\_crab': w=60; h=40; vx = -CONFIG.BASE\_SCROLL\_SPEED\*speedMult; y=height-50; break;

                case 'seagrass': w=20; h=80+Math.random()\*80; vx = -CONFIG.BASE\_SCROLL\_SPEED\*speedMult; y=height; break;
...
            if(gameSpeed < CONFIG.MAX\_SPEED\_MULTIPLIER) gameSpeed += CONFIG.DIFFICULTY\_RAMP;

            const scrollSpeed = CONFIG.BASE\_SCROLL\_SPEED \* gameSpeed;
...
                    else { p.isAnchored = false; p.energy -= CONFIG.ENERGY\_CAMO\_DRAIN; }
...
                    p.energy -= CONFIG.ENERGY\_DECAY\_RATE \* 0.1;

                    p.vx \*= 0.8; p.vy \*= 0.8;

                } else {

                    p.vx += dx \* CONFIG.PLAYER\_ACCEL;

                    p.vy += dy \* CONFIG.PLAYER\_ACCEL;

                    p.vx \*= CONFIG.PLAYER\_FRICTION;

                    p.vy \*= CONFIG.PLAYER\_FRICTION;



                    const spd = Math.hypot(p.vx, p.vy);

                    if(spd > CONFIG.PLAYER\_MAX\_SPEED) {

                        p.vx = (p.vx/spd)\*CONFIG.PLAYER\_MAX\_SPEED;

                        p.vy = (p.vy/spd)\*CONFIG.PLAYER\_MAX\_SPEED;
...
                    p.energy -= (CONFIG.ENERGY\_DECAY\_RATE + spd \* CONFIG.ENERGY\_MOVE\_COST);
...
            const cycle = frameCount % CONFIG.TRAWL\_INTERVAL;

            const warningEl = document.getElementById('hud-warning');



            if(cycle === CONFIG.TRAWL\_INTERVAL - CONFIG.TRAWL\_WARNING\_DURATION) {
...
                net.speed = CONFIG.TRAWL\_SPEED\_BASE \* (1 + (gameSpeed-1)\*0.5);
...
                const netH = height \* CONFIG.TRAWL\_HEIGHT\_RATIO;
...
            const spawn = CONFIG.SPAWN\_RATES;
...
                    const type = Math.random()>0.5 ? 'predator\_tuna' : 'predator\_crab';

                    const y = type==='predator\_tuna' ? Math.random()\*(height-150) : height-30;
